...
| ~~`/bugfix` skill~~ — **done** | Shipped as `/fix`: a diagnosis in front of step 1 and a Defect block in the plan file, not a second pipeline — steps 2 to 10 are the same for a fix, and the step numbers are load-bearing in the hooks. `CLAUDE.md` has the fix-round table; the two `fix/guard-git-parsing` rounds are the evidence, having improvised every part of it. | — |
