...
| `plan-critic` agent | Both existing subagents feed steps 4 to 6, after the code exists. Step 2 is where design errors are cheapest to catch and has no adversarial read at all — and it now matters more, because nobody reads the plan again until step 6. Planned as round 2 of the autonomous-pipeline work, together with a second `test-designer` brief at step 5 and three brainstorming lenses at step 8. | `/feature` |
