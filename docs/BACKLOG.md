...
| ~~`/bugfix` skill~~ — **done** | Shipped as `/fix`: a diagnosis in front of step 1 and a Defect block in the plan file, not a second pipeline — the step numbers and markers are the same, six steps read the Defect block where one is present, and the hooks are untouched because the numbers they carry are load-bearing. `CLAUDE.md` has the fix-round table; the two `fix/guard-git-parsing` rounds are the evidence, having improvised every part of it. | — |
